...
def verify(alg, message, signature, public_key, level):
    if alg == "RSA":
        ...
        sec = "Dilithium2" if level == 1 else "Dilithium3" if level == 2 else "Dilithium5"
...
for level in range(3,0,-1):
    ...
    start_time = time.time()
    while time.time() - start_time < 1:
        verify(alg, msg, signature, vk, level) 
        ver+=1
    
    print("%s Can run %s Keygen, \t%s Signing and \t%s Verification in a Second " % (sec, kyg, sig, ver)) 
